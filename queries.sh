...
echo "$($PSQL "SELECT DISTINCT(name) FROM (SELECT teams.name FROM teams 
...
echo "$($PSQL "SELECT games.year, teams.name FROM teams 
FULL JOIN games ON teams.team_id = games.winner_id 
WHERE round='Final' 
ORDER BY games.year, teams.name")"

echo -e "\nList of teams that start with 'Co':"
echo "$($PSQL "SELECT name FROM (SELECT teams.name FROM teams 
INNER JOIN games ON teams.team_id = games.winner_id
